...
def download_messages_csv(
    ...
    for m in messages:
        ...
        # avoid treating SQLAlchemy Column as bool; check for None explicitly
        receiver = db.query(User).filter(User.id == m.receiver_id).first() if m.receiver_id is not None else None
        writer.writerow([
            m.id,
...
def admin_login(data: dict, db: Session = Depends(get_db)):
    ...
    # ensure password is a string before passing to verifier
    if not user or password is None:
        raise HTTPException(status_code=401, detail="Invalid credentials")

    # mypy/pylance may think user.password_hash is a Column[str]; cast to str
    hashed = str(user.password_hash)  # type: ignore
    if not verify_password(password, hashed):
        raise HTTPException(status_code=401, detail="Invalid credentials")

    # convert Column[bool] to bool for conditional checks
